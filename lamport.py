...
while(count < eventos):
    ...
    if p_envia == p_recebe:
        ...
        print "\n# TIPO: Evento Interno"
        print "# Mensagem = %s" % relogio[p_envia][-2]
        print "# Relogio Logico de Processo%s = %s\n" % (p_envia, relogio[p_envia][-1])
		
    else:
        relogio[p_envia].append(relogio[p_envia][-1] + 1)
        print "\n# TIPO: Evento Entre Processos"
        print "# Mensagem = %s" % relogio[p_envia][-1]
        print "# Relogio Logico de Processo%s = %s" % (p_envia, relogio[p_envia][-1])

        if relogio[p_envia][-1] > relogio[p_recebe][-1]:
            relogio[p_recebe].append(relogio[p_envia][-1] + 1)
            print "# Relogio Logico de Processo%s = %s\n" % (p_recebe, relogio[p_recebe][-1])
        else:
